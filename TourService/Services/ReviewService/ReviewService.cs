...
using TourService.Models.User.Requests;
        ...
        public async Task<long> AddReview(AddReviewRequest review)
        {
            ...
            try
            {
                ...
                if(transaction.SaveAndCommit())
                {
                    ...
                    Tour tour = await _unitOfWork.Tours.FindOneAsync(x=>x.Id==review.TourId);
                    tour.Rating=(await _unitOfWork.Reviews.GetAll().Where(x=>x.TourId==review.TourId).ToListAsync()).Average(x=>x.Rating);
                    return result.Id;
                }
